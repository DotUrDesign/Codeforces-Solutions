...
double pi = 3.14159, npi = -3.14159;
 ...
 << npi << endl;
*/
pair<ll,ll> dfs(ll node,vector<ll>adj[],vector<bool>check,string& s,ll& ans)
{
    if(check[node])
        return {0,0};
    ll b=0,w=0;
    ...
    for(auto it:adj[node])
    {
        if(!check[it])
        {
            pair<ll,ll> p = dfs(it,adj,check,s,ans);
            b = p.first;
            w = p.second;
        }
    }
