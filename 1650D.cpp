...
void leftRotate(vector<ll>& nums,ll n, ll k)
{
    k %= n;
...
void solve()
{
    // Think of this question in a reverse order. Like how can we make the original array by the given array.
    ll n; cin>>n;
    vector<ll>nums(n);
    for(ll i=0;i<n;i++)
        cin>>nums[i];
