...
{
    ll n; cin>>n;
    vector<ll>vis(n-1,-1), nums(n);
    for(ll i=0;i<n;i++)
        cin>>nums[i];

    /*
        Case 1 --> nums[i] == nums[i+1]  =>  You can take any range from low to high. No need to change the range.
        Case 2 --> nums[i] > nums[i+1]   =>  Range = (ceil of avg of nums[i] & nums[i+1] , high)
        Case 3 --> nums[i] < nums[i+1]   =>  Range = (low , floor of avg of nums[i] & nums[i+1])
    */
    ...
    for(ll i=0;i<n-1;i++)
    {
        ...
        {
            ll avg = (nums[i] + nums[i+1] + 1)/2;
            low = max(low, avg);
        }
        else if(nums[i] < nums[i+1])
        {
            ll avg = (nums[i] + nums[i+1])/2;
            high = min(high, avg);
        }
    }
    if(low <= high)
