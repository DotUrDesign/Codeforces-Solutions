...
void solve()
{
    ...
    ll ans = 1e15;
    ans = min(ans, abs(nums[a][0] - nums[b][0]) + abs(nums[a][1] - nums[b][1]));

    ll mina = 1e15;
    for(ll i=0;i<k;i++)
    {
        mina = min(mina , abs(nums[a][0] - nums[i][0]) + abs(nums[a][1] - nums[i][1]));
    }
    ll minb = 1e15;
    for(ll i=0;i<k;i++)
    {
        minb = min(minb, abs(nums[b][0] - nums[i][0]) + abs(nums[b][1] - nums[i][1]));
    }
