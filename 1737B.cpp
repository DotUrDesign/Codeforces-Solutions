...
void solve()
{
    ...
    ll val1 = (ll)(sqrt(l));
    ll val2 = (ll)(sqrt(r));
    ll ans = max(0ll,(val2 - val1 - 1)*3);
    if(val1*val1 >= l and val1*val1 <= r)
        ans++;
    if((val1+1)*(val1) >= l and (val1+1)*(val1) <= r)
        ans++;
    if((val1+2)*(val1) >= l and (val1+2)*(val1) <= r)
        ans++;
    
    if(val1 != val2)
    {
        if(val2*val2 >= l and val2*val2 <= r)
            ans++;
        if((val2+1)*(val2) >= l and (val2+1)*(val2) <= r)
            ans++;
        if((val2+2)*(val2) >= l and (val2+2)*(val2) <= r)
            ans++;
    }
    cout<<ans<<endl;
}
