...
void solve()
{
    ...
    ll ans = -1;
    // brute force --> j is the number of liars
    for(ll j=0;j<=n;j++)
    {
        ll cnt = 0;
        for(ll i=0;i<n;i++)
        {
            // if some person is saying that there are more than "j" liars, then he himself is a liar.
            if(nums[i] > j)
                cnt++;
        }

        // if the number of liars satisfies the value of j, then j is the answer.
        if(cnt == j)
        {
            ans = j;
            break;
