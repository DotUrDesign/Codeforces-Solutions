...
void solve()
{
    ...
    ll ans = 0;
        for(ll st=0;st<n;st++)
        {
            for(ll i=st;i<=(n-2-st);i++)
            {
                char ch = 'a';
                ch = max(ch, arr[st][i]);
                ch = max(ch, arr[i][n-1-st]);
                ch = max(ch, arr[n-1-st][n-1-i]);
                ch = max(ch, arr[n-1-i][st]);

                ans += abs(ch - arr[st][i]);
                ans += abs(ch - arr[i][n-1-st]);
                ans += abs(ch - arr[n-1-st][n-1-i]);
                ans += abs(ch - arr[n-1-i][st]);
                // cout<<ans<<" ";
            }
