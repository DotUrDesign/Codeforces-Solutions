...
ll func(vector<ll>& nums,int n,int i,int j,int year)
{
    if(i == j)
        return year * nums[i];
    
    ll x = year * nums[i] + func(nums,n,i+1,j,year+1);
    ll y = year * nums[j] + func(nums,n,i,j-1,year+1);
    return max(x,y);
}
