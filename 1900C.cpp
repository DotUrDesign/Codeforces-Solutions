...
ll _dp(ll n, string& s, vector<ll>& left, vector<ll>& right, ll node, vector<ll>& dp)
{
    // base case 
    ...
    if(left[node] != 0)
        l = _dp(n, s, left, right, left[node], dp);
    if(right[node] != 0)
        r = _dp(n, s, left, right, right[node], dp);
    if(s[node-1] == 'L')
        return dp[node] = min(l, 1 + r);
    else if(s[node-1] == 'R')
        return dp[node] = min(1 + l, r);
    else 
        return dp[node] = min(1 + l, 1 + r);
}
...
void solve()
{
    ...
    vector<ll>dp(n+10, -1);
    ll ans = _dp(n, s, left, right, 1, dp);
    cout<<ans<<endl;
}
int main()
{
